...
window.bg_canvasImage.src = "img/achterGrond2.png";

// startscherm achtergrond plaatje
window.startSchermImage 	= new Image();
window.startSchermImage.src = "img/hoofdscherm.png";

// eindscherm achtergrond plaatje bji verlies
window.eindSchermVerlies 	= new Image();
window.eindSchermVerlies.src = "img/eindschermverlies.png";

// eindscherm achtergrond plaatje bji winst
window.eindSchermWinst	= new Image();
window.eindSchermWinst.src = "img/eindschermwinst.png";
...
// Boolean welke vertelt of het spel gewonnen is
window.winst = new Boolean;
window.winst = false;
...
// alle variabelen worden hersteld naar de orginele waarde
...
function drawBackground() {
	//teken achtergrond
		if(gameStatus == 'spel'){
			...
			if (achtergrondX < - 2100 )
			{
				achtergrondX = 	700;
				
			}

			if (achtergrondX2 < -2100 )
			{
				achtergrondX2 = 700;
			}
...
function drawTelefoon(){
	...
	if(telefoonBoolean == true)
	{
 	...
 	if (telefoonBoolean == false && gameStatus == 'spel'){
...
function scoreDisplay() {
						...
						//tekenen van text op het canvas
						canvasContext.globalAlpha=0.8;   // Full opacity
...
function drawStartScherm(){
	...
	canvasContext.drawImage(startSchermImage, 0, 0);	
	canvasContext.drawImage(autoImage, 148 ,157,117,63,300, 260,168,92);
}
...
function drawEindscherm(){
    ...
    // het tekenen van het eindscherm
    canvasContext.drawImage(eindSchermVerlies, 0, 0);	
	canvasContext.drawImage(autoImage, 148 ,157,117,63,300, 260,168,92);

	// tekenen van de laatste behaalde score
	canvasContext.globalAlpha=0.8;   // Full opacity
	canvasContext.beginPath();
   	canvasContext.rect(15, 300, 250, 80);
    canvasContext.fillStyle = 'white';
    canvasContext.fill();
    canvasContext.globalAlpha=1;   // Full opacity
    canvasContext.lineWidth = 5;
    ...
    canvasContext.strokeStyle = 'black';
    canvasContext.lineWidth = 1.5;
    canvasContext.fillText("Gereden meters:  " 	+ ((seconden - 4)* 10) ,20,330);
    canvasContext.fillText("Aantal berichten:  " 	+telefoonOverleefd,20,350);
	}
	...
	if (winst == true){
		    // het tekenen van het eindscherm
		     ...
		     clearInterval(intervalGame);
  		     clearInterval(intervalTeller);  
   			 canvasContext.drawImage(eindSchermWinst, 0, 0);	
			 canvasContext.drawImage(autoImage, 148 ,157,117,63,300, 260,168,92);
	}
}
	 ...
	 if (e == null) { // ie 
			...
			if(keycode == 13 && gameStatus == 'start'){ // escape, close box, esc 
              ...
              restartGame();
          	  ...
          	  // enter verwerking om van het start scherm naar het spel scherm te gaan
			if(keycode == 13 && gameStatus == 'eind'){ // escape, close box, esc 
              //what you nedd
              gameStatus = 'spel';
